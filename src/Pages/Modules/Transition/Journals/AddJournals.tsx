...
const AddJournals: React.FC = () => {
  const [rows, setRows] = useState<RowData[]>([
    { description: "", account: "", debit: 0, credit: 0, status: "not-added" },
  ]);
    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="relative">
          <div className="gap-2 grid md:grid-cols-3 mt-3 w-full">
            <div className="space-y-1">
              <label htmlFor="date" className="ml-1 text-xs">
                Date
              </label>
              <DatePicker onChange={onChange} className="w-full h-[40px]" />
            </div>
                    ...
                    <tr key={index}>
                      ...
                      <td className="border-gray-200 dark:border-gray-700 px-2 md:px-6 py-4 border-b whitespace-no-wrap">
                        <div className="text-sm leading-5">
                          ...
                          >
                            {items.map((itm) => (
                              <option key={itm.key} value={itm.label}>
                                {itm.label}
                              </option>
                            ))}
                          </select>
        ...
        <div className="flex md:flex-row flex-col justify-between items-start">
          <Button type="primary" className="rounded" onClick={handleAddField}>
            + Add Field
          </Button>
          <div className="div">
