...
export default function EditExpensePage() {
    ...
    const {
        ...
        queryFn: async () => {
            ...
            const res = await fetch(
                `${import.meta.env.VITE_BASE_URL}transaction/income/get-income/${expenseId}`,
                {
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: user._id,
            ...
            if (!res.ok) {
                ...
                throw new Error(errorData.message || 'Failed to fetch Income');
            }
    ...
    useEffect(() => {
        if (!isValidId(expenseId)) {
            message.error('Invalid Income ID');
            setLoading(false);
            return;
        }
    ...
    const { mutate: updateExpense, isPending: isUpdating } = useMutation({
        mutationFn: async (payload: any) => {
            const res = await fetch(
                `${import.meta.env.VITE_BASE_URL}transaction/Income/update-income`,
                {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const onFinish = async (values: any, status: 'Approved' | 'Draft') => {
        ...
        if (!isValidId(expenseId)) {
            return message.error('Invalid Income ID');
        }
        ...
        if (!expenseCategories.every(cat => cat.category)) {
            return message.error(
                'Please select a category for all income items.'
            );
        }
    ...
    if (loading || isFetching) return <div>Loading expense data...</div>;
                        ...
                        >
                            <Row gutter={24}>
                                ...
                                >
                                    ...
                                    {/* Expense Categories - Using SAME logic as AddExpense */}
                                    <Card className="mb-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-colors">
