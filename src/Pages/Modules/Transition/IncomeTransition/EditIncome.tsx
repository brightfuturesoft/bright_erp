...
const EditExpense: React.FC = () => {
    ...
    useEffect(() => {
        if (!user?._id || !isValidId(id)) {
            message.error('Invalid income ID or User not loaded');
            setLoading(false);
            return;
        }
        ...
        const fetchExpense = async () => {
            try {
                const res = await fetch(
                    `${import.meta.env.VITE_BASE_URL}transaction/income/get-income/${id}`,
                    {
                        headers: {
                            'Content-Type': 'application/json',
                            Authorization: user._id,
                ...
                if (res.ok && !result.error) {
                    ...
                    message.error(result.message || 'Failed to fetch income');
                }
            } catch (err) {
                console.error(err);
                message.error('Error fetching income');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmit = async () => {
        ...
        // Validation
        if (!expenseFrom) return message.error('Please select income From');
        ...
        try {
            ...
            console.error(err);
            message.error('Something went wrong while updating income');
        }
    };
                ...
                <Content className="p-6">
                    ...
                    <div className="mt-4">
                        ...
                        <Select
                            ...
                            className="mb-4 w-full"
                            placeholder="Select income From"
                        >
                            {accounts
