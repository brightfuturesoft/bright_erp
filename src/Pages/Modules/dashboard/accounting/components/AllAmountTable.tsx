...
const TransactionsTable: React.FC<{ currencySymbol: string }> = ({
  ...
  const [filter, setFilter] = useState<"all" | "bank" | "cash" | "mobile_bank">(
    "all"
  );

  const filteredUsers =
    filter === "all" ? users : users.filter((user) => user.ac_type === filter);
    ...
    <div className="mt-8">
      <div className="mb-4">
        ...
        <select
          ...
          value={filter}
          onChange={(e) =>
            setFilter(e.target.value as "all" | "bank" | "cash" | "mobile_bank")
          }
        >
