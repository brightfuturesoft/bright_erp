...
const Rack: React.FC = () => {
    ...
    const fetchRacks = async () => {
        ...
        try {
            ...
            setError(error.message || 'Error fetching racks');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAddRack = async () => {
        ...
        try {
            ...
            setError(error.message || 'Error adding rack');
        }
    };
    ...
    const handleDeleteRack = async (id: string) => {
        if (window.confirm('Are you sure you want to delete this rack?')) {
            try {
                await axios.delete(
                    `${getBaseUrl}/inventory/ware_house/rack/${id}`
                );
                fetchRacks();
            } catch (error: any) {
                setError(error.message || 'Error deleting rack');
            }
        }
    };
    ...
    const handleStatusUpdate = async (id: string, newStatus: string) => {
        try {
            console.log(`Updating status for rack ${id} to ${newStatus}`);
            // Use dedicated status update endpoint like Area and Cell
            const response = await axios.put(
                `${getBaseUrl}/inventory/ware_house/rack/${id}/status`,
                {
                    status: newStatus,
                }
            );
                    ...
                    <tbody>
                        {racks.map(rack => (
                            ...
                            >
                                ...
                                <td className="px-4 py-2">{rack.warehouse}</td>
                                <td className="px-4 py-2">
                                    <select
                                        ...
                                        className="px-3 py-1 rounded bg-green-500 text-white border-none"
                                    >
