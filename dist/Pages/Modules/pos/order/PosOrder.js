...
const PosOrder = () => {
    ...
    const sub_total_amount = useCallback(orders => {
        let total = 0;
    ...
    // Tax calculation
    const sub_total_tax = useCallback(orders => {
        return orders.reduce((acc, order) => acc + (order.tax_amount || 0), 0);
    }, []);
    // Grand total calculation
    const grand_total_amount = useCallback(orders => {
    ...
    return _jsxs(Section, {
        ...
        children: [
            ...
            _jsx(TableFilter, {}),
            _jsx(DataTable, { orders: orders_data }),
        ],
    });
};
